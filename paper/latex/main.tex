...
\textbf{Goal.} Strict serializability of committed effects plus confinement of the executor to its authorized policy envelope, achieved \emph{online}, at the commit gate, with no trusted hardware and no replication.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{fig1-architecture.png}
\caption{Architecture. The broker owns $\kappa$, the storage handle, and the single-writer lease; the cell gets a UDS and nothing else.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{fig2-read-trap.png}
\caption{One read trap.}
\label{fig:readtrap}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{fig4-conflict-windows.png}
\caption{The two conflict windows of a limited ascending scan.}
\label{fig:windows}
\end{figure}
...
\item
  the \textbf{cell-facing write path} (S9): \texttt{Commit}/\texttt{Abort} verbs on the UDS protocol, where the broker resolves the session, verifies the capsule seal, enforces the declared set against the sealed observations (B-SUBSET), derives conflict windows from every sealed certificate (never from a cell claim), and drives the \texttt{CommitFence} seam; any structured commit/abort answer closes the session (one session = one transaction attempt); and the V8 mutation syscalls (\texttt{1.0/insert}, \texttt{1.0/shallowMerge}, \texttt{1.0/replace}, \texttt{1.0/remove}) that grow the write set inside the cell with read-your-own-writes semantics and no store authority (\texttt{crates/ipc/src/\{lib.rs,bin/aster\_brokerd.rs\}}, \texttt{crates/v8cell/src/lib.rs}). In postgres mode the broker's epoch comes from \texttt{acquire\_lease} at boot and is stamped into every minted session; a mint-time context claiming any other epoch is refused.
\end{itemize}
...
\textbf{Table 1: positioning by trust locus $\times$ mechanism $\times$ guarantee $\times$ cost.}
