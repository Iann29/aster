...
| **Eleven** integration proofs against real Postgres: epochs never reuse; CE 3.9 write skew impossible sequentially AND under real concurrency (exactly one of two racing fences commits); stale epoch can't append after failover; GC blocks on in-flight fence pin + coverage enforced; replay commits as a second transaction; phantom insert conflicts with Exhausted window but NOT past a Boundary window; absence/tombstone reads conflict on later writes; MVCC point/prefix semantics; retention watermark clamps to the log tip (commit-admission liveness regression) | `crates/store-postgres/tests/write_plane_it.rs` (11 `#[test]`s, listed by name in the file). **Count note:** the S12 briefing said "13 integration proofs"; the base branch had 10 (commit c615bd9's message also says 10); commit 885ef9b added `retention_watermark_clamps_to_log_tip` → 11. The paper says eleven. Sibling slices are adding tests this round — re-count at merge | IMPLEMENTED |
...
| ~13.4k lines of Rust, 8 crates, 197 tests | measured in-tree: `git ls-files 'crates/**/*.rs'` piped to `xargs wc -l` = 13,405 (tests included); grep-count `#[test]`+`#[tokio::test]` = 197; crates: broker, capsule, convex-codec, host, ipc, runner, store-postgres, v8cell | MEASURED (2026-07-16, branch v0.7-fix-paper @ 75263cb — siblings adding tests in parallel; re-measure at merge) |
...
- **LOC:** README says "roughly 10k"; measured 13,405 incl. tests on this tree
  (2026-07-16, v0.7-fix-paper @ 75263cb). Paper says "roughly thirteen thousand…
  (13.4k measured, tests included)". Re-measure at submission.
- **Count-carrying lines to re-stamp at merge** (sibling slices are adding tests in
  parallel; numbers below are correct for THIS tree only): paper abstract ("roughly
  thirteen thousand"), §1.1 C4 (same), §5 first ¶ (13.4k / 197 tests), §5 codec bullet
  (14-of-18), §3.2 canonical-encoding ¶ (14-of-18), §5 fence ¶ ("eleven integration
  proofs"), and this ledger's §3 rows (codec 18/14, proofs 11, LOC/tests row) + the
  first note above.
