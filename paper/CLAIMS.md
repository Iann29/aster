...
| ~19.0k lines of Rust, 8 crates, 263 tests | measured in-tree: `git ls-files 'crates/**/*.rs'` piped to `xargs wc -l` = 18,992 (tests + the S10 bench bin included); grep-count `#[test]`+`#[tokio::test]` = 263; crates: broker, capsule, convex-codec, host, ipc, runner, store-postgres, v8cell | MEASURED (2026-07-17, branch v0.7-write-path, post-round-2 repair kit — re-measure at submission) |
...
- **LOC:** README says "roughly 10k" (stale there too); measured 18,992 incl. tests
  + the S10 bench bin on this tree (2026-07-16, v0.7-s10 @ 8cb09c7; 17,727 without
  `crates/ipc/examples/`). Paper says "roughly nineteen thousand… (19.0k measured,
...
- **Count-carrying lines** (re-stamped 2026-07-16 at the S10 landing —
  LOC 18,992 / 263 tests / eighteen fence proofs / codec 18-total-14-rejection):
