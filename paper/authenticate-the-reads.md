...
| Warm-read marginal ((TKsame − T1)/199) | ~0.005 ms/read (timer-resolution bound) |
...
| + window validation over ~1k-event (s, h], w = 1 / 10 / 50 | 4.59 / 5.81 / 6.61 ms | ≤7.3 | 200 each |
| Conflict-abort (declared point, interposed write) | **1.75 ms** | 1.98 | 100 |

The fence is durability-bound, not validation-bound. The blind commit's 3.5 ms is dominated by the synchronous WAL flush at COMMIT — which is why an *abort is half the price of a commit*: same lease lock, same horizon, same conflict scan, no flush. Point validation is one extra round trip whose cost is flat in p (an `= ANY` probe over an empty conflict window; the p=200 median landing below p=1 is run-to-run noise, ~0.4 ms). Window validation pays the DISTINCT-key scan over the (s, h] population (~1.1 ms for ~1k events) plus in-committer interval matching that becomes visible by w=50 (+2 ms). 280 serial commits/s is the fsync-per-commit ceiling of one connection on this disk; group commit is the standard lever and nothing in the fence forbids it, but we report the measured serial figure only.
