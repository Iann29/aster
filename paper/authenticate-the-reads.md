...
- **C4 — system.** Aster: roughly eighteen thousand lines of Rust across eight crates, running unmodified `npx convex deploy` bundles end-to-end against the same Postgres schema the upstream Convex backend writes. Measured on the real pipeline: ~0.04 ms of authentication apparatus per read at small capsules, linear in capsule size (EQ2), and 6.5 ms / 153 tx/s for a full fenced mutation from an untrusted cell (Section 6).
...
**The loop is closed.** Since S9, mutations travel the path the theorem describes: JavaScript in the untrusted cell issues mutation syscalls that build the write set in-cell, the consumption ledger becomes the declared set, and the `Commit` verb carries (sealed capsule, declared reads, write set) over the UDS socket to the broker, which reduces it to a `FenceInput` and lets the Postgres fence decide. The gated end-to-end proof commits a JS-born write set through the real fence and aborts an interleaved conflicting cell (`pg_v8_mutation_write_set_commits_and_interleaved_conflict_aborts`, `crates/ipc/src/bin/aster_brokerd.rs`); the write-path benchmark this unlocked is Section 6.3, and its harness is versioned in-repo (`bench/run-v07.sh`, `crates/ipc/examples/bench_v07.rs`, results committed under `bench/results/v07/`).
...
The fence is durability-bound, not validation-bound. The blind commit's 3.5 ms is dominated by the synchronous WAL flush at COMMIT — which is why an *abort is half the price of a commit*: same lease lock, same horizon, same conflict scan, no flush. Point validation is one extra round trip whose cost is flat in p (an `= ANY` probe over an empty conflict window; the p=200 median landing below p=1 is run-to-run noise, ~0.4 ms). Window validation pays the DISTINCT-key scan over the (s, h] population (~1.1 ms at ~1k events) plus in-committer interval matching; the sweep's three w-phases share one growing log (~1.0k events at w=1, ~1.6k by w=50 — each committed sample appends), so the +2 ms from w=1 to w=50 is an upper bound that conflates window count with population growth. 280 serial commits/s is the fsync-per-commit ceiling of one connection on this disk; group commit is the standard lever and nothing in the fence forbids it, but we report the measured serial figure only.

**Table 7 — the full theorem loop, one transaction per iteration (N=500 after 30 warmup).**
...
**Prototype seams the theorem does not hide.** Two v0.7 plumbing limitations are stated rather than papered over. (i) *Boot-pinned snapshots:* brokerd pins one snapshot and one lease epoch at launch, and the demo read plane serves the Convex schema, which never observes `aster.log` commits — so a `Conflict`/`RetentionViolated` outcome's "retry from a fresh snapshot" contract is dischargeable in v0.7 only by relaunching the broker; a live integration (the F9 decision, Section 5) would refresh snapshots against the very plane the fence appends to. (ii) *Document-id aliasing:* the store accepts two spellings of the same row (Convex IDv6, plus a raw wire form used by native callers and tests), while every layer above — capsule keys, consumption ledger, write set, conflict scan — keys by the string; a transaction mixing spellings for one document would evade read-your-own-writes and pairwise conflict detection. A real Convex bundle cannot produce this (JS only ever holds IDv6 strings), so the hazard is confined to hand-rolled native clients; broker-side canonicalization at the hydrate/commit seam is the named fix.
