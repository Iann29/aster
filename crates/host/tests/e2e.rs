use aster_capsule::{doc_with_i64, DocumentId, TenantId, DeploymentId, Value};
...
fn query_mutation_and_action_traverse_capsule_pipeline() {
    ...
    assert_eq!(query_outcome.execution.traps, 1, "the second key was hydrated as a read trap");
    ...
    let action_outcome = host.execute(action).expect("action should create an effect fence");
    assert_eq!(action_outcome.execution.output, Value::Text("effect:send-webhook".to_string()));
...
fn two_mutation_results_over_same_snapshot_cannot_both_commit() {
    ...
    host.store().seed(DocumentId::new("counters/main"), doc_with_i64("value", 1));
    ...
    let first = cell.execute(host.store(), invocation.clone()).expect("first execution");
    let second = cell.execute(host.store(), invocation).expect("second execution");

    host.store()
        .commit(base_ts, &first.read_set, &first.write_set)
        .expect("first commit wins");
    let loser = host.store().commit(base_ts, &second.read_set, &second.write_set);
